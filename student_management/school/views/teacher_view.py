...
class GradeView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["is_homeroom"] = self.is_homeroom
        context["subject_name"] = self.subject_name
        context["formset"] = formset
...
@login_required
def achievement_view(request, class_id, semester_id):
    class_students = ClassStudent.objects.filter(
        classroom_id=class_id, semester_id=semester_id
    )
    ...
    for class_student in class_students:
        ...
        for subject in subjects:
            grade = Grade.objects.filter(
                classstudent=class_student, subject=subject
            ).first()
            grades.append(grade.final_grade)
        class_student.grades = grades
        ...
        if classification:
            class_student.gpa = classification.gpa
            class_student.rank = classification.rank
        else:
            class_student.gpa = None
            class_student.rank = None
    ...
    if sort_by == "asc":
        class_students = sorted(
            class_students, key=lambda x: x.gpa if x.gpa is not None else 0
        )
    elif sort_by == "desc":
        class_students = sorted(
            class_students,
            key=lambda x: x.gpa if x.gpa is not None else 0,
            reverse=True,
        )
    ...
    context = {
        ...
        "is_homeroom": True,
        "subjects": subjects,
        "achievements": class_students,
        "semesters": semesters,
        "current_semester_id": semester_id,
    }

    return render(request, "school/achievements/achievement_list.html", context)
